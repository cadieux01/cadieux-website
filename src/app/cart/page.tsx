...
function chip(selected: boolean) {
  return {
    ...
    color: selected ? "#FBF3D4" : "rgba(2,70,40,0.7)",
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#C0C8CE", position: "relative", overflowX: "clip" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "100px clamp(28px,8vw,120px) 120px" }}>
        <ScrollReveal>
          <h1 data-stagger style={{ margin: "0 0 48px", fontFamily: "var(--font-heading)", fontSize: "clamp(52px,12vw,96px)", fontWeight: 300, color: "#024628", letterSpacing: "0.02em", lineHeight: 1 }}>
            Your Cart
          </h1>
        </ScrollReveal>
        ...
        {cart.length === 0 ? (
          ...
          <>
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {cart.map((item, i) => (
                <div key={i} style={{ borderTop: "1px solid rgba(2,70,40,0.15)", padding: "28px 0", display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <p style={{ margin: 0, fontFamily: "var(--font-heading)", fontSize: "clamp(18px,4vw,28px)", fontWeight: 300, color: "#024628", lineHeight: 1.2, maxWidth: "70%" }}>{item.name}</p>
                    <p style={{ margin: 0, fontFamily: "var(--font-heading)", fontSize: "clamp(18px,4vw,28px)", fontWeight: 300, color: "#024628" }}>₹{item.price * item.qty}</p>
                  </div>

...
function buildMonthCells(y: number, m: number, today: Date, now: Date): Cell[][] {
  ...
  for (let r = 0; r < 6; r++) {
    ...
    for (let c = 0; c < 7; c++) {
      ...
      // A future date always has at least one bookable slot (tomorrow's
      // evening window is ≥16 h out from any "now"); today is the only
      // date that can go fully-blocked once enough of the day has
      // passed. Skip the check on past dates.

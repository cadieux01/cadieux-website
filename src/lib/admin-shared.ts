...
export type AdminOrderRow = {
  id: string;
  /** Human-facing order number ('OLF7') assigned by the DB trigger
...
export type AdminSubscriptionRow = {
  id: string;
  /** OLS reference, from public.subscriptions_number_seq. Until 2026-09-14
   *  subscriptions shared orders_number_seq, so one interleaved series ran
   *  across both tables; the renumber split them into two counters, and the
   *  prefix swap later that day settled them as OLF (orders) and OLS
   *  (subscriptions). Nullable only so a partial projection can't render
